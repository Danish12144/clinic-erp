...
| Everything else (staff management, appointments, EMR, pharmacy, lab, billing, CRM, ...) | Not started |
...
**Staff account provisioning is a real invite/accept-token flow, not an Owner-set password** — `staff_invites` (migration `0005`) stores only a hashed opaque token; `POST /api/v1/doctors` creates the `User` (status `INVITED`, no `password_hash`) and returns the raw token in the response body outside `production` only (`debug_invite_token`, same placeholder-delivery pattern as OTP's `debug_code` — no Communications module exists yet to deliver it by email/SMS for real). The invited doctor calls `POST /api/v1/doctors/accept-invite` with `{clinic_slug, token, password}` to set their password and flip to `ACTIVE`, then logs in normally via the existing `/api/v1/auth/staff/login`. A resend (`POST /api/v1/doctors/{user_id}/invite/resend`) deletes any still-pending invite row for that user first, so an old leaked/previous token stops working the moment a new one is issued. `user_branch_assignments` and `staff_invites` are deliberately generic to any staff role, not doctor-specific — the Staff Management module (built next) should reuse both rather than duplicating them.
