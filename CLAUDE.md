...
| Doctor Management (provisioning, profile, branch assignment) | ✅ Done — `backend/app/modules/doctors/`, migration `0005`. A read-only "doctor directory" for non-Owner/non-Doctor roles is explicitly out of scope — see the module's own doc note in `docs/DATABASE-SCHEMA.md` |
