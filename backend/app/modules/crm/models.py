...
`CommChannel`/`CommStatus`/`CommunicationLog` moved to
`app.modules.notifications.models` in migration 0024, once Notifications
became the true owner of the outbox concept (every business-trigger
dispatch goes through `dispatch_notification` there, not just this
module's own stub-insert) — this module now imports them from there
directly (see `app/modules/crm/repository.py`/`service.py`/`schemas.py`),
no re-export left behind here.
"""
...
from app.modules.notifications.models import CommunicationLog  # noqa: F401 -- referenced by the `reminder` relationship's string annotation below
